...
$(document).ready(async function() {
    ...
    function showNextQuestion() {
        ...
        $('#chinese-options').empty();
        options.forEach(option => {
            $('#chinese-options').append(`
                <button class="btn btn-outline-primary option-btn mb-2" data-definition="${option.definition}">
                    ${option.definition}
                </button>
            `);
        });
    ...
    $('#submit-answer').off('click').on('click', function() {
        ...
        const selectedDefinition = selectedOption.data('definition');
        const currentWord = currentWords[currentIndex];
        ...
        if (isCorrect) {
            ...
            $('#feedback').removeClass('alert-success').addClass('alert-danger')
                .html(`❌ 回答错误。正确答案是：${currentWord.definition}`).show();
            // 答错时重置该单词的连续正确次数
